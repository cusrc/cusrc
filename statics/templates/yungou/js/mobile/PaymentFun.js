    ...
    var b = function() {
        ...
        var f = function(y) {
            ...
            if (y > 0) {
                ...
                p.attr("sel", "1").attr("class", "z-pay-mentsel").next("span").html('金币支付<em class="orange">' + y + ".00</em>元（账户金币：" + d + " 元）")
				checkpay='money';
				banktype='nobank';
            } else {
                p.attr("sel", "0").attr("class", "z-pay-ment").next("span").html('金币支付<em class="orange">0.00</em>元（账户金币：' + d + " 元）")
            }
        };
        var k = function(y) {
            e = y;
            ...
            if (w + g >= x) {
			    ...
			    if (shopnum != -1) {
					if (shopnum == 0) {
                        ...
                        $.getJSON(Gobal.Webpath+"/mobile/cart/paysubmitajax/"+checkpay+"/"+banktype+"/"+x+"/"+t+"/"+submitcode,{
                            uid:   localStorage.getItem('uid'),
                            ushell:    localStorage.getItem('ushell'),
                            Mcartlist:JSON.parse(localStorage.getItem('Cartlist'))
                        },function (res) {
                            layer.closeAll();
                            ...
                            layer.msg('支付结果', {
                                ...
                                ,yes: function(index){
                                    ...
                                    if(res.status){
                                        ClearShopCart() ;
                                        //  本初链接需要取消注释
                                        window.location.href="../../../index.php/mobile/home/userbuylist.html";
                                    }else{
                                      window.location.href="../../../index.php/mobile/mobile/lottery.html";
                                    }

                                }
                            });
                            try{
                                if(res.info.indexOf("成功")!=-1){
                                    ClearShopCart() ;
                                }
                            }catch (e21313) {
